...
sillyvec <- c("PDUCK13")  # no otolith data for PSIWAS15 :(
...
data_qry <-
  paste0(
    "SELECT * FROM AKFINADM.GEN_SAMPLED_FISH_TISSUE WHERE SILLY_CODE IN (",
...
dataAll0 %>% 
  count(SILLY_CODE, PK_TISSUE_TYPE)  # sample size by silly code and tissue type, should only be hearts and occasionally other random tissues
...
write_csv(
  x = dataAll0,
  file = paste0(
    "data/LOKI_tissue_GEN_SAMPLED_FISH_TISSUE_",
    paste(sillyvec, collapse = "_"),
    "_",
