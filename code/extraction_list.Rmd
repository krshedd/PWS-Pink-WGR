...
# Additional paired data for Will

10/12/22
Will Hemstrom, Mark Christie's new post-doc, is requesting additional paired data for his WGR analyses.
...
Just kidding, 10/14/22, abandoning the code below, going to start a different notebook for cleanliness `individual_paired_data_join.Rmd`
...
Read in data for the fish we selected

```{r}
(extraction_erb_2017 <- readr::read_csv(file = "../output/extraction_selection_PERB17.csv"))
```

### Stockdale 2016 natural

```{r}
(extraction_PSTOCK16_natural <- readr::read_csv(file = "../output/extraction_PSTOCK16_natural.csv"))
```

```{r}
(extraction_PSTOCK16_natural_v2 <- readr::read_csv(file = "../output/extraction_PSTOCK16_natural_v2.csv"))
```

### Stockdale 2015 natural

```{r}
(extraction_PSTOCK15_natural <- readr::read_csv(file = "../output/extraction_PSTOCK15_natural.csv"))
```

### Hogan 2016 natural

```{r}
(extraction_PHOGAN16_natural <- readr::read_csv(file = "../output/extraction_PHOGAN16_natural.csv"))
```

### Hogan 2015 natural

```{r}
(extraction_PHOGAN15_natural <- readr::read_csv(file = "../output/extraction_PHOGAN15_natural.csv"))
```
...
```{r}
( <- readr::read_csv(file = "../output/extraction_P"))
```
...
Will need SillySource + DWB_Position for data keys
Get length and otolith ID, will get location from `riverdist` .csv later on
