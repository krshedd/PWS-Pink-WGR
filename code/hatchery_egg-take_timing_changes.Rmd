...
author: "Kyle Shedd"
date: "2024-03-26; updated 2024-04-05 write out cleaned eggtake data; update 2024-04-11 with Lorna's cleaned data"
output:
  html_notebook:
    theme: united
    toc: yes
...
readr::write_csv(x = eggtake_clean, file = "../output/PWS_hatchery_eggtake_cleaned_20240405.csv")
```
...
eggtake_clean %>% 
  dplyr::rowwise() %>%
  dplyr::mutate(p50_julian_manual = floor(mean(c(start_julian, end_julian))),
                p50_julian_check = p50_julian == p50_julian_manual) %>% 
  dplyr::count(p50_julian_check)
```

Whew, all okay, so long as I used `floor` instead of `mean`.

Sent this `PWS_hatchery_eggtake_cleaned_20240405.csv` to Lorna Wilson (PNP planning/permitting coordinator - Juneau) on 2024-04-05 to double check the excel data I received from Heather Scannel (PWS Seine AMB - Cordova) with the official PNP database. Want to make sure that I have the correct information for Will Hemstrom and Mark Christie for the WGR *lrrc9* manuscript.
...
Check out founding brood years.
```{r}
eggtake %>% 
  dplyr::filter(`Founding brood?` == 1)
```

Need to fix AFK 1976, take earliest start and latest end dates.
```{r}
eggtake_clean <- eggtake %>% 
  dplyr::select(hatchery:duration) %>% 
  dplyr::filter(year == 1976 & hatchery == "AFK") %>% 
  dplyr::mutate(start_date = min(start_date),
                end_date = max(end_date),
                start_julian = lubridate::yday(start_date),
                end_julian = lubridate::yday(end_date),
                p50_julian = floor((start_julian + end_julian) / 2),
                duration = end_julian - start_julian + 1,
                p50_date = lubridate::as_date(p50_julian, origin = "1976-01-01")) %>% 
  dplyr::distinct() %>% 
  dplyr::bind_rows(eggtake %>% dplyr::filter(!(year == 1976 & hatchery == "AFK"))) %>% 
  dplyr::arrange(hatchery, year)
```
